...
chinese_data = nil
http.get("https://raw.githubusercontent.com/2510714739/Legit/main/Helper_chinese", function(success, response)
	if not success or response.status ~= 200 then
		return
	end

	chinese_data = loadstring(response.body)()
end)
...
local old_font = surface.create_font("Verdana", 15, 700, {0x010, 0x080})
local font_size = ui.new_slider("VISUALS", "Other ESP", "Helper Font Size", 0, 100, 15, true, "%", 1, {[0] = "Off"})
ui.set_callback(font_size, function()
	old_font = surface.create_font("Verdana", ui.get(font_size), 700, {0x010, 0x080})
end)
					...
					if wx_top ~= nil then
						...
						if location_set.distance_width_mp > 0 then
							...
							for i=1, #lines do
								...
								if chinese_data ~= nil then
									surface.draw_text(_x, _y, r, g, b, a, old_font, chinese_data[string.upper(text)] or text)
								end
				...
				for i=1, #location_set_closest do
					...
					if location.viewangles_target ~= nil then
						...
						if wx ~= nil then
							...
							if chinese_data ~= nil then
								surface.draw_text(wx_topleft+target_size, wy, r_m, g_m, b_m, a_m*visible_alpha, old_font, chinese_data[string.upper(location.name)] or location.name)
							end

							--renderer.text(wx_topleft+target_size, wy, r_m, g_m, b_m, a_m*visible_alpha, "bd", 0, name)
							if description ~= nil then
								if chinese_data ~= nil then
									surface.draw_text(wx_topleft+target_size, wy+title_height, math.min(255, r_m*1.2), math.min(255, g_m*1.2), math.min(255, b_m*1.2), a_m*visible_alpha*0.92, old_font, chinese_data[string.upper(description)] or description)
								end
